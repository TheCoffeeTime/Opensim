...
namespace OpenSim.Region.OptionalModules.Example.BareBonesNonShared
{
    ...
    public class RayTracer
    {
        ...
        System.IO.StreamWriter progressBarFile;    //A file which contains progress (in %) of the raytracer calculation
        public bool RayTraceIsCalculated = false;  //Has the method RayTrace been run? (i.e. Has it started the ray tracer?)
                ...
                if (m_prims[i] is SceneObjectGroup)
                {
                    ...
                    sog.ForEachPart(delegate(SceneObjectPart part)
                    {
                        if (Vector3.Distance(part.AbsolutePosition, transmitter.RootPart.AbsolutePosition) <= MAX_DISTANCE || part.Name.CompareTo("Ry") == 0)
                        {
                            ...
                            if (m_prims[i].Name.CompareTo("Test") == 0)
                            {
                                m_log.DebugFormat("[---------------------!!!]" + Vector3.Distance(part.AbsolutePosition, transmitter.RootPart.AbsolutePosition));
                            }
                        }
                    });
                }
        ...
        /// <summary>
        /// This method can only be used when RayTraceIsCalculated is true. 
        /// It check the given index and return the most sensible one. E.g. if index = -1, then it will return the last element
        /// If index = list length, then it will return the first element (index = 0). Else just return the given index.
        /// </summary>
        public int checkAndGetCorrectPathIndex(int noOfReflection, int index)
        {
            if (RayTraceIsCalculated)
            {
                if (index < 0)
                {
                    //Return index = list length - 1, i.e. the last element of the list
                    return pathHits[noOfReflection].Count() - 1;
                }
                else if (index >= pathHits[noOfReflection].Count)
                {
                    return 0;
                }
                else
            ...
            {
                throw new Exception("checkAndGetCorrectPathIndex() cannot be called if RayTracer hasn't started raytracing");
            }
        }
        /// <summary>
        ...
        void updateProgressBar(float percentage)
        {
            ...
            progressBarFile = new System.IO.StreamWriter(@"C:\Users\Thanakorn\Documents\wamp\www\www\controlBoard\rayTraceLog\progressLog.txt");
            progressBarFile.WriteLine(percentage.ToString());
            progressBarFile.Close();

        }
        /// <summary>
        /// Update one log files so that the remote control can read how many path for each reflections. 
        /// </summary>
        void updateRayTraceLogFile()
        {
            progressBarFile = new System.IO.StreamWriter(@"C:\Users\Thanakorn\Documents\wamp\www\www\controlBoard\rayTraceLog\0_ref.txt");
            progressBarFile.WriteLine(pathHits[0].Count());
            progressBarFile.Close();

            progressBarFile = new System.IO.StreamWriter(@"C:\Users\Thanakorn\Documents\wamp\www\www\controlBoard\rayTraceLog\1_ref.txt");
            progressBarFile.WriteLine(pathHits[1].Count());
            progressBarFile.Close();

            progressBarFile = new System.IO.StreamWriter(@"C:\Users\Thanakorn\Documents\wamp\www\www\controlBoard\rayTraceLog\2_ref.txt");
            progressBarFile.WriteLine(pathHits[2].Count());
            progressBarFile.Close();

            progressBarFile = new System.IO.StreamWriter(@"C:\Users\Thanakorn\Documents\wamp\www\www\controlBoard\rayTraceLog\3_ref.txt");
            progressBarFile.WriteLine(pathHits[3].Count());
            progressBarFile.Close();

            progressBarFile = new System.IO.StreamWriter(@"C:\Users\Thanakorn\Documents\wamp\www\www\controlBoard\rayTraceLog\4_ref.txt");
            progressBarFile.WriteLine(pathHits[4].Count());
            progressBarFile.Close();

            progressBarFile = new System.IO.StreamWriter(@"C:\Users\Thanakorn\Documents\wamp\www\www\controlBoard\rayTraceLog\5_ref.txt");
            progressBarFile.WriteLine(pathHits[5].Count());
            progressBarFile.Close();

            progressBarFile = new System.IO.StreamWriter(@"C:\Users\Thanakorn\Documents\wamp\www\www\controlBoard\rayTraceLog\isInitialised.txt");
            progressBarFile.WriteLine("ready");
            progressBarFile.Close();
        }
        /// <summary>
        ...
        void saveRayTracerModel()
        {
            ...
            {   
                ...
                foreach (KeyValuePair<string, PathFromTxToRy> path in pathHits[i])
                {
                    ...
                    {
                        rayTraceModel += Math.Round(ray.Value.startPoint.X, 2) + ","
                                         + Math.Round(ray.Value.startPoint.Y, 2) + ","
                                         + Math.Round(ray.Value.startPoint.Z, 2) + "_"
                                         + Math.Round(ray.Value.endPoint.X, 2) + ","
                                         + Math.Round(ray.Value.endPoint.Y, 2) + ","
                                         + Math.Round(ray.Value.endPoint.Z, 2) + "_";

                    }//foreach
                    rayTraceModel += "#";
                }//foreach
            }//for
            m_commsMod.DispatchReply(scriptID, 1, rayTraceModel , "");
        }
        public void retriveFromNoteCard(string rayTracerModelInString)
        {
        }
        /// <summary>
