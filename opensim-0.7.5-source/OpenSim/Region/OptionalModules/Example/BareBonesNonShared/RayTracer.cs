...
namespace OpenSim.Region.OptionalModules.Example.BareBonesNonShared
{
    ...
    public class RayTracer
    {
        ...
        /// that equation. The limitation is that is cannot 
        /// </summary>
        ...
        public ReflectionPoint findReflectionPoint(Vector3 pointFrom, Vector3 pointTo, SceneObjectPart part)
        {
            ...
            foundX = false; foundY = false; foundZ = false;

            //Check what is known. x y or z, and initialise them. Use this to find the rest of the variables.
            ...
            if ((intersection.normal.X != 0 && intersection.normal.Y != 0) ||
                (intersection.normal.X != 0 && intersection.normal.Z != 0) ||
                (intersection.normal.Y != 0 && intersection.normal.Z != 0))
            {
                //This is not currently supported by my function. The normal of the prim isn't one of the axis.
            ...
            else
            {
                ...
                else if (intersection.ipoint.Z != 0)
                {
                ...
                {
                    throw new Exception("findReflectionPoint(): no normal");
                }//else
            }//else

            //if x is fixed, then use x to find y
            //if y is fixed, then use y to find x
            //if z is fixed, then use it to find x
            if (foundX)
            {
        ...
        /// </summary>
        /// <param name="transmitterPos">The point where the ray start</param>
        /// <param name="endPoint">The point where the ray end</param>
        public void drawPlaneRay(Vector3 transmitterPos, Vector3 endPoint)
        {
            ...
            double rayLength = (transmitterPos - endPoint).Length();
            Vector3 midPoint = (transmitterPos + endPoint) / 2;
            Vector3 vectorFromStartToEnd = endPoint - transmitterPos;
            ...
            addObjectToTheWorld("Rays", midPoint, dimension, rotation, PrimType.Cylinder, transmitterPos);
