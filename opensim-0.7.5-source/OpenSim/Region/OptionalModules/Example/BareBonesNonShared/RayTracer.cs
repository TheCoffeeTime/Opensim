...
namespace OpenSim.Region.OptionalModules.Example.BareBonesNonShared
{
    ...
    public class RayTracer
    {
        ...
        List<Vector3> allRayPos;                   //A list of vectors for all ray position. This will be used to check for repition.
        ...
        public string setOfVariables;              //Set of variables which are set by a user on the ray tracer controller. 
        ...
        public string ModelName;                   //Name of the model for this ray tracer.
        ...
        public bool RayTraceIsCalculated = false;  //Has the method RayTrace been run?
        ...
        int initialiseVariables(string setOfVariables)
        {
            ...
            rayScript = System.IO.File.ReadAllText(@EssentialPaths.getRayScriptPath());
            return countVariable;
        }
        /// <summary>
        /// </summary>
        public void RayTrace(bool onlyUpToOneReflection, Scene _scene, string _setOfVariables, UUID _scriptID)
        {
            Initialise(_scene, _setOfVariables, _scriptID);
        ...
        /// <summary>
        /// Apply Snells law to calculate the angle of refraction/reflection. 
        /// Note: Applying Snells'law to a ray, you can either get refraction or reflection. 
        /// </summary>
        ...
        /// <summary>
        /// Draw a path which has no of relfection = the given noOfReflection its index = elementIndex. 
        /// </summary>
        /// <param name="noOfReflection">The reflection number of the path needed to be drawn</param>
        /// <param name="elementIndex">The index of this path. </param>
        public void drawRayPath(int noOfReflection, int elementIndex)
        {
            m_log.DebugFormat("[noOfRef]" + noOfReflection.ToString());
            m_log.DebugFormat("[elementIndex]" + elementIndex.ToString());
            //If ray tracer has been calculated, then try drawing a given path. Else, do nothing for now. 
            if (RayTraceIsCalculated)
            {
                //If there is at least one element in the list for this reflection. 
                if (pathHits[noOfReflection].Count() != 0)
                {
                    string[] keys = pathHits[noOfReflection].Keys.ToArray();
                    //To make sure that it doesn't go over the bound
                    if (elementIndex < keys.Length)
                    {
                        drawRayPath(pathHits[noOfReflection][keys[elementIndex]]);
                    }
                }
            }
        }
        ...
        /// <param name="name">Name/ID of this ray</param>
        ...
        /// <summary>
        /// When running a ray tracer, there will be a progress bar showing the user. 
        /// This method will write log update into a progressBar file so that
        /// it is readable from the client side. The progress mbar animation is actually done using javascript. 
        /// </summary>
        void updateProgressBar(float percentage)
        {
            System.IO.StreamWriter progressBarFile;
            percentage = (float)Math.Floor(percentage);
            progressBarFile = new System.IO.StreamWriter(@EssentialPaths.getProgressBarLogPath());
            progressBarFile.WriteLine(percentage.ToString());
            progressBarFile.Close();

        }
        /// <summary>
        /// After the ray tracer has been 'calculated' it will be written to string and send back to the user who
        /// has started the ray tracer so that this model can be used later (without recalculating everything again.)
        /// Each path is seperate by '#', each point is seperate by '_', and each coordinate (x, y, z) is seperate by ','
        /// e.g. p1_p2_p3#p4,p5_p6_p7 where each 'p' has 3 attributes (x, y, and z).
        /// 
        /// The save is actually done on the client side using the viewer. This method sends back a message to the viewer
        /// to write to the note card. To retrive the model, we use rayTracerModelInString()
        /// </summary>
        void saveRayTracerModel()
        {
            string rayTraceModel = "RTModel//" + setOfVariables + "//";
            ...
            {   
                ...
                {
                    //Starting point
                    rayTraceModel += Math.Round(transmitter.RootPart.AbsolutePosition.X, 2) + ","
                                    + Math.Round(transmitter.RootPart.AbsolutePosition.Y, 2) + ","
                                    + Math.Round(transmitter.RootPart.AbsolutePosition.Z, 2) + ","
                                    + runningPowerRT + "_";
                    //For rach ray(in this path)
                    foreach (KeyValuePair<string, RayAsPathSection> ray in path.Value.rayList)
                    {
                        rayTraceModel +=   Math.Round(ray.Value.endPoint.X, 2) + ","
                                         + Math.Round(ray.Value.endPoint.Y, 2) + ","
                                         + Math.Round(ray.Value.endPoint.Z, 2) + ","
                                         + Math.Round(ray.Value.minPower, 2) + "_";

                    }//foreach
                    rayTraceModel += "#";
                }//foreach
            }//for
            System.IO.StreamWriter newModel;
            newModel = new System.IO.StreamWriter(@EssentialPaths.getRTModelPath() + ModelName + ".txt");
            newModel.WriteLine(rayTraceModel);
            newModel.Close();
        }
        /// <summary>
        /// After the ray tracer has finished, we need to refresh the remote control page with a 'correct'variables 
        /// (such as max path for each reflection, currently selected model name and available model).
        /// This method is called at the end of creating new ray tracer model.
        /// </summary>
        /// <param name="saveTheCurrentModelName">Do you want to save the name of this current model?
        /// True this trace model is newly created or False if this ray trace model is read from a file</param>
        void packAndSendGETMessages(bool saveTheCurrentModelName)
        {
            string GETMessage = "NewRTModel//SelectedModel=" + ModelName + "&";

            //Get all the available models
            string allModels = System.IO.File.ReadAllText(@EssentialPaths.getModelListPath());
            if (saveTheCurrentModelName)
            {
                allModels += ModelName + "_";
            }
            System.IO.File.WriteAllText(@EssentialPaths.getModelListPath(), allModels);
            GETMessage += "ExistedModels=" + allModels + "&";

            GETMessage += "0R=" + pathHits[0].Count() + "&";
            GETMessage += "1R=" + pathHits[1].Count() + "&";
            GETMessage += "2R=" + pathHits[2].Count() + "&";
            GETMessage += "3R=" + pathHits[3].Count() + "&";
            GETMessage += "4R=" + pathHits[4].Count() + "&";
            GETMessage += "5R=" + pathHits[5].Count() + "&";

            m_commsMod.DispatchReply(scriptID, 1, GETMessage, "");
        }
        /// <summary>
        /// Check if the given model contains the 'same' transmitter as the current one by compare their position
        /// </summary>
        /// <param name="listOfPath">Section3 from the string model</param>
        /// <returns>Does this current transmitter have the same poisition as the previous one?</returns>
        bool sameTransmitter(string listOfPath)
        {
            string[] pathList = listOfPath.Split('#');
            string[] pointList = pathList[0].Split('_');
            //Transmitter position
            string[] pointElements = pointList[0].Split(',');
            Vector3 currentPoint = new Vector3(float.Parse(pointElements[0]), float.Parse(pointElements[1]), float.Parse(pointElements[2]));
            if (currentPoint.X == (float)Math.Round(transmitter.RootPart.AbsolutePosition.X, 2) &&
                currentPoint.Y == (float)Math.Round(transmitter.RootPart.AbsolutePosition.Y, 2) &&
                currentPoint.Z == (float)Math.Round(transmitter.RootPart.AbsolutePosition.Z, 2))
            {
                return true;
            }
            else
            {
                m_log.DebugFormat("[CURRENT POS]" + transmitter.RootPart.AbsolutePosition);
                m_log.DebugFormat("[MODEL POS]" + currentPoint);
                return false;
            }
        }
        /// <summary>
        ...
        /// <summary>
        /// A ray with power. A ray has a start point and end point. This ray has a start power and end power. 
        /// End power is simply (Start power - power loss in 'air propergation')
        /// </summary>
